module LocationBoxing
  ...
  def self.getWithinQueryMongo(lon, lat)
    lateq = "{ $and: [nw_lat : { $gt:#{lat} }, se_lat : { $lt:#{lat} }] }"
  ...
  # the long way. Always the shortest longitudinal distance (< 180 degrees)
  ...
  def enlargeBox(box,buffer)
    ...
    nw_lon = nw_lon - ((1/Math.cos(nw_lat) * LON_PER_FOOT) * buffer/2.0).abs
    se_lon = se_lon + ((1/Math.cos(se_lat) * LON_PER_FOOT) * buffer/2.0).abs
  ...
  def getCenter(c1,c2)
    ...
    if dist > 0
      ...
      return getDestinationPoint(c1, dist/2, bearing)
    else
      return c1
    end
  end
  ...
  # of the geodesic hypotenuse between the two points. It is directional.
  ...
  #
  # This function returns the distance from the line
  #
  ...
  def onLine1(c1,c2,buffer,point)
    ...
    if equalCoordinates?(c1,c2)
      return equalCoordinates?(c1,point) || buffer > getGeoDistance(c1,point)
    end
  ...
  #
  # This function returns the closest distance that the point is near the
  # path.
...
#           # This is just integrity check. It can be eliminated.
  ...
  def distanceToSegmentSquared(point, c1,c2)
    x0 = point[0]
    y0 = point[1]
    x1 = c1[0]
    y1 = c1[1]
    ...
    if (dx + dy == 0)
      ...
      along = ((dx * (x0 - x1)) + (dy * (y0 - y1))) / (dx * dx + dy * dy)
    end
    x = nil
    y = nil
    if(along <= 0.0)
  ...
  def distanceToSegment(point, c1, c2)
    result = distanceToSegmentSquared(point, c1, c2)
    result[:distance] = Math.sqrt(result[:distance])
    result[:geo_distance] = getGeoDistance(point, result[:coords])
    return result
  end

  POLE = 20037508.34

  def inverseMercator(c1)
    c2 = []
    c2[0] = 180 * c1[0] / POLE
    c2[1] = 180 / Math::PI * (2 * Math.atan(Math.exp((c1[1] / POLE) * Math::PI)) - Math::PI / 2)
    return c2
  end

  def forwardMercator(c1)
    c2 = []
    c2[0] = c1[0] * POLE / 180
    y = Math.log(Math.tan((90 + c1[1]) * Math::PI / 360)) / Math::PI * POLE
    #c2[1] = Math.max(-20037508.34, Math.min(y, 20037508.34))
    c2[1] = [-20037508.34, [y, 20037508.34].min].max
    return c2
  end
  ...
  # This function returns the distance from the line
  #
  def distToSegment(c1,c2, c3)

    if equalCoordinates?(c1, c2)
      return {
          :distance => equalCoordinates?(c1, c3) ? 0.0 : getGeoDistance(c1, c3),
          :along => 0.0,
          :lon => c1[0],
          :lat => c1[1],
          :coords => c1
    ...
    # Convert the points to mercator and get the answer
    mc1 = forwardMercator(c1)
    mc2 = forwardMercator(c2)
    mc3 = forwardMercator(c3)

    ans = distanceToSegmentSquared(mc3, mc1,mc2)

    coords = inverseMercator(ans[:coords])

    return {
        :distance => getGeoDistance(c3, coords),
        :mercator_distance => Math.sqrt(ans[:distance]), # In Mercator Projection
        :along => ans[:along],
        :lon => coords[0],
        :lat => coords[1],
        :coords => coords
  ...
  def getPossible(vps, coord, buffer, average_speed)
    ...
    path = [vp1]
    points = []
    # We start with a negative distance in case the coord is the beginning of the TL, since
    # we only add answers if the distance is greater than the last distance if on a segment.
    # This avoids adding answers for multiple points that are the same.
    distance = -1
    while (!vps.empty?) do
      vp2 = vps[0]
      vps = vps.drop(1)
